...
- For `messageKind: "image"`, do **not** inline base64 image bytes in chat responses. Call `wechat image get <messageId> --json`, parse `absolutePath`, then use the host agent's file/image Read capability on that path. If the command returns `cdn-expired` or `needs local-decrypt RE (issue #X)`, report that the image cannot be fetched by the current CDN-only path.
