...
wechat history <chat> --json | jq '.[] | {kind: .message_kind, text: .display_text}'
```
...
`wechat audio setup`) / `failed` / `skipped_svr_id_zero`.

For a single voice file outside `history`:

```bash
SVR=$(wechat history <chat> --json | jq -r '.[] | select(.message_kind=="audio") | .server_id' | head -1)
...
`wechat audio get <svr_id>` writes the raw `.silk` bytes from
`media_*.db.VoiceInfo.voice_data` (pure SQLCipher BLOB read; no LLDB / heap
scan / CDN). To play / share without going through `audio transcribe`:
