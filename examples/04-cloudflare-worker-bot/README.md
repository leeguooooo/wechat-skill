...
2. **一次性跑 tunnel setup**（你自己的 CF free 账号，OAuth 授权即可）：
   ```bash
   wechat tunnel setup
   ```
   命令会引导你完成 Cloudflare OAuth → 创建 named tunnel → 启动 tunnel 进程。  
   成功后输出类似：
   ```
   ✓ tunnel created: <uuid>.cfargotunnel.com
   ✓ tunnel process running (pid 12345)
   ```
   **把 `<uuid>.cfargotunnel.com` 记下来**（即 `TUNNEL_URL`）。
