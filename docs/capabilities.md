...
| 收语音（raw SILK_V3） | ✅ v1.13.21 | `wechat audio get <svr_id>` | 直接读 `media_*.db.VoiceInfo.voice_data` BLOB,strip 1-byte Tencent prefix,落 `~/.wechat/audio-cache/<svr_id>.silk` |
...
| `wechat doctor --image` | ✅ v1.14.1/2 | `wechat doctor --image [--json]` | image AES key 自检(magic byte 验);Plan A 不适用时验 heap-scan 是否可用,避免假绿 |
| `wechat init --literal-scan` | ✅ v1.14.0 | `wechat init --literal-scan` | 版本无关 fallback key 提取(WeChat 大版本升级 RE 完成前临时跑) |
| history 引用 / 合并转发 / 文件 | ✅ v1.14.0 | `wechat history` 自动渲染 | 引用回复(refermsg)→ `[引用 X 「abc」] ↳ Y: txt`;合并转发(type=19)出 `forwarded_messages` 数组;文件(type=6)出 `media` 元数据 |
