...
| **history 自动转写语音** | ✅ v1.13.25 | `wechat history <chat>` (默认) | agent 看群历史时,语音消息自动转成文字塞进 `display_text` + `media.transcript`,**告别 `[语音消息]` 占位破上下文**。SHA-256 内容寻址 cache 二次访问 0.4s 全 hit;`media.transcript_status` 字段诊断;`--no-transcribe` opt-out / `--transcribe-model small` 切小模型 |
