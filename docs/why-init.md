...
2. **用 LLDB 一次性抓取**：仅在写入瞬间触发并立刻 detach。**不会改 WeChat 二进制 / 不会写任何东西 / 不需要 sudo（除了 v1.8.16+ 的一次性系统前置）/ 不会对 wechat.dylib 重签名**（其他同类方案会改你的 WeChat.app，我们不改）。
...
Tencent 热更可能在「自动升级」关闭的情况下仍然把 `wechat.dylib` 换成新版本（Sparkle / WeChat 内置更新路径），新版本里关键路径会偏移。

v1.8.13 起，init 默认带 `--calibrate`，自动在新 dylib 上重新适配并缓存。**每个新 dylib SHA 上自动校准。不再需要手动 `--force` 或换 dmg。**
