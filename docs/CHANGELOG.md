...
issue #8(history 让 agent 看到图片消息)**部分实现** — 设计 B 默认 + A opt-in。

**默认行为**(免费):`wechat history` 每条 `message_kind=image` row 现在自动检查 `~/.wechat/media-cache/<md5>.{jpg,png,gif}` 是否存在 — **之前你 `wechat image get` 跑过的所有图自动出现 `media.local_path` 字段**,agent 直接 Read 就能多模态消费。0 daemon RPC,几个 stat 调用,~ 1ms per image row。

**`--auto-fetch-images` opt-in**(慢):cache miss 时调 daemon 的 `image_heap_extract` RPC 主动 heap-scan(per-row 5-7s 冷启)。命中落 `~/.wechat/media-cache/` + 填路径。heap miss(用户没在 WeChat UI 点开过)设 `status=heap_miss`。**100 条历史 10 条图 cold scan ≈ 50-70s**,所以 opt-in。

**`media.local_path_status` 字段**:`cached` / `fetched` / `not_cached` / `heap_miss` / `no_md5` / `daemon_unreachable`。agent 看 status 就知道路径从哪来,可以决定是否要改用其他 fetch 方式。

**为什么 issue #8 留 open:**

image 跟 audio 不一样:audio 走 `transcript 文字塞进 display_text`,agent 直接读;image 没等价 UX,只能填本地路径让 agent 用 Read 工具读图。所以这次只是 plumbing,UX 上比 audio 差一截。完整解决需要 daemon 端整合 OCR / LLM vision 描述塞 `display_text`,scope 更大,留作未来工作。

## v1.13.35 — 2026-05-06

修 codex round-1 v1.13.22 review SHOULD #1 跨命令共性问题(memory `project_with_auth_json_bypass_todo.md` 接力点):**订阅过期 / token 失效时,所有 `--json` 命令现在会先打一行 JSON envelope 到 stdout 再返错**,不再 silent fail。

**之前的痛点:** v1.13.20 9 轮 ping-pong 修了 `wechat send --json` 的 ok:bool envelope,但其他 12 个 read 命令(`history` / `sessions` / `contacts` / `search` / `image` / `audio` / `digest` / `favorites` / `members` / `stats` / `unread` / `recalled`)的 `with_auth` 仍在命令体之前跑 `ensure_valid_subscription`。subscription 错 → 直接返 `CommandError` → main.rs `eprintln!` stderr → 命令的 `--json` 处理压根没跑。agent 跑 `wechat history --json | jq .ok` 时 stdout 空 → jq parse fail。stderr 有 "subscription expired" 但 agent pipeline 一般不抓 stderr。

**修法:**

- `with_auth` 现是 `with_auth_maybe_json(false, f)` 的 thin wrapper
- 新增 `with_auth_maybe_json(wants_json, f)`:subscription 错时如果 `wants_json=true`,先 `println!` 一行 `{"ok":false,"error":"...","exit_code":2}` 到 stdout,再 return Err。main.rs 仍正常打 stderr + 设 exit code(跟 `send.rs` sentinel pattern 一致)
- `AudioArgs` / `ImageArgs` 加 `wants_json()` 方法 drill into subcommand variants(audio / image 是 subcommand 结构)
- 13 个 dispatch 站点改用 `with_auth_maybe_json` 传 `args.json` 或 `args.wants_json()`
- 不带 `--json` 的命令(`Export` / `Listen` / `Friendship`)仍走 `with_auth`(= `maybe_json(false)`)

**实测**(临时 `mv ~/.wx-rs/auth*.json` 模拟过期):

```bash
$ wechat history filehelper -n 1 --json
{"error":"尚未激活订阅...","exit_code":2,"ok":false}
$ echo $?
2
```

agent friendly,人类 stderr 仍清晰。

## v1.13.34 — 2026-05-06

修 codex round-4 review v1.13.33 找到的 3 SHOULD + 3 NICE,无 BLOCKER。`wechat digest` CLI surface 定型期 polish:

- `digest` help 文案 + module doc 明确 **"不是 LLM 摘要,是 chronological compact dump"**(LLM 端跑摘要,本工具只 pre-process 给 prompt)。命令名保留(v1.13.33 已 ship,改名要 alias 复杂)。
- `digest` 默认跳 `recalled` 跟 `system` 同等待遇。v1.13.33 文案说跳"撤回 / kick noise"但代码只 `kind == system`,撤回还出。`--include-system` 现在同时控制两者。
- `digest` 加 `--chat <CHAT>` flag 跟 `history` / `image get` 接口对齐。positional / `--chat` 二选一。
- `digest --no-transcript-marker`:`history` 有,`digest` 之前 hardcoded 加 marker 没 opt-out。parity 修上。
- `digest --kind <KIND[,...]>` filter:跨日 audio-only 摘要等场景直接 `wechat digest --kind audio`。复用 `history` 同款 csv 解析。
- 删 unused `anyhow` import + `_silence_unused` 死代码 helper。

测试:213 全 lib pass。e2e:`digest --kind audio` 跨 2 天群历史跑通,撤回消息默认不出。

**Today 收手节奏:** v1.13.34 是当前 codex round-4 review 全清。用户提了「查看撤回消息」新 feature → 起 v1.13.35 单独立项,然后一起公告。

## v1.13.33 — 2026-05-06

新增 **`wechat digest <chat>`** — agent / LLM 友好的群历史摘要预处理(codex CLI 反馈 #6)。修 codex round-3 v1.13.31 review 剩的 2 NICE。今晚 audio + history + agent UX 这条线 **codex 找不到问题** 收手。

**`wechat digest <chat>`**:跟 `wechat history` 共享 enrich pipeline(transcribe / strip-xml / created_at),但输出层是日分隔的紧凑文本,直接喂给 LLM 做摘要:

```
=== 2026-05-05 ===
11:42 qq461609248: 文班的防守太强了
11:43 me: 7 盖帽？
11:43 qq461609248: 8个吧
12:01 me: [image]
13:29 qq461609248: 🔊 这PP付费啊付费拉倒了
14:29 qq461609248: @Leo
14:29 qq461609248: 🔊 过去给咱俩买那个停板那个连接你还有吗
...
```

**默认值优化**:`--since "2 days ago"` / `-n 500` / 自动 transcribe(`--transcribe-model auto`)/ 自动 `--strip-xml`(防 url/appmsg context bomb)。`--include-system` 反向显示撤回 / kick noise(默认跳)。`--json` 精简 schema `{day, time, sender, kind, text}` 给 agent 二次解析。

**Round-3 NICE 收尾:**

- `doctor.rs::check_ok` 加 `debug_assert!` 防 `query_ready` / `send_ready` 计算时 check 名字 typo 静默返 false。debug build / tests 大声 panic;release graceful。
- `wait_for_message_db_change` empty-paths fallback 触发时 stderr 一行 log 给诊断。如果未来几个 release 0 hit 可直接删 fallback。

测试:213 全 lib pass。e2e:digest 输出 13 行群历史紧凑文本带 🔊 marker / `[image]` 占位 / `@Leo` 等 mention,对人对 agent 都友好。

**今晚 13 个 release(v1.13.21 → v1.13.33)累计:**
- audio MVP → ASR pipeline → history 自动转写 → codex 多轮 ping-pong → warmup auto-retry → daemon stale socket cleanup → agent-friendly fields(query/send_ready, created_at)→ history strip-xml/fields → digest 命令
- codex 累计抓出 3 BLOCKER + 31 SHOULD + 13 NICE 全采纳处理

## v1.13.32 — 2026-05-06

`wechat history` 加 2 个 agent 必备选项 + 修 codex round-3 review v1.13.31 找到的 1 SHOULD + 1 NICE。

**`wechat history --strip-xml`**(codex CLI 实测反馈 #3):把 `message_kind in {url, appmsg, chathistory, mini_program, voip, location, attachment, contact}` 的 `message_content`(常常 1-15KB XML body)折叠成 ~80B 占位 `"<stripped: N chars XML; see display_text + url_link + media for parsed fields>"`。结构化字段(`display_text` / `url_link` / `media`)agent 已经能从 daemon 拿到,raw XML body 只是 context bomb。**实测 200 条群历史 285KB → 145KB(50% 压缩)**,单条 attachment 从 2712B → 81B。

**`wechat history --fields a,b,c`**(codex CLI 实测反馈 #3 同上):JSON 输出的 `extra` HashMap 白名单过滤。例:`--fields display_text,message_kind,created_at` 只留这 3 个 extra field,其余(`media` / `url_link` / `chat_display_name` 等)全 drop。top-level scalar(`local_id` / `server_id` / `create_time` 等)始终保留(它们是 struct field,不在 extra 里)。

**Round-3 SHOULD `is_orphan_socket` pid reuse 防御**:v1.13.31 只用 `kill(pid, 0)` 检测,pid 被其他进程 recycle 后误判 alive,持续 transient error loop。加 `ps -p <pid> -o comm=` 验证 process name 含 `wechatd`,非 wechatd → orphan。macOS pid reuse 周期长但理论风险消除。

**Round-3 NICE orchestrate log 同步**:v1.13.31 改 `SEND_ROW_TIMEOUT` 60→120s 但 4 处 `eprintln!` / `error_message` 字符串还写 "60s",误导排障。一并改 120s。

**Skip**(留下次):codex round-3 NICE 2 项(`check_ok` name guard / empty-paths fallback over-trigger)— 风险都是保守方向(false 不是 true),低 ROI;`wechat digest` 大 feature(codex CLI 反馈 #6)。

测试:213 全 lib pass。e2e: kill -9 daemon → ping 自动恢复(pid-reuse guard 加上仍 work);`history --strip-xml` 200 row 285KB→145KB;`--fields` 白名单生效。

## v1.13.31 — 2026-05-06

修 codex round-2 review v1.13.30 找到的 3 SHOULD + 1 NICE,加 codex CLI 实测反馈 2 项(daemon stale socket auto-cleanup + doctor query_ready/send_ready)。

**codex CLI 实测反馈:**

- **daemon stale socket auto-cleanup**:之前 daemon `kill -9` / OOM 后 `/tmp/wechatd-501.sock` 残留,client 撞 `ECONNREFUSED` 沿用 transient error,用户要手动 `rm` 才恢复。v1.13.31 新增 `is_orphan_socket()` 检测 pidfile + `kill -0`,orphan 自动 `rm` + 走 spawn 路径。**e2e 实测**:`kill -9 daemon` → 任意命令(`wechat ping` / `wechat history`)自动检测 stale socket → 清理 + spawn 新 daemon → 无感执行。
- **`wechat doctor --json` 加 `query_ready` / `send_ready`**:agent 不用 parse `status` string(jargon: `needs_send_verify` vs `needs_init`)。两个顶层 bool 直接判断:
  - `query_ready` = config + key + DB + daemon ok(history / sessions / contacts 等读路径可用)
  - `send_ready` = `query_ready` + AX trusted + 至少一次 send 验证过

**codex round-2 review v1.13.30 SHOULD:**

- **doctor `other_models` 没验证 size**:0-byte 占位文件(下载中断)会假绿让 Auto 假装能选,但实际 whisper 跑不了。`model_is_present()` 检查 size > 0;display 也用 `WhisperModel::Auto.resolve_installed()` 真实告知 Auto 会选哪个,不再 hardcode `other_models[0]`(顺序不一定符合 Auto 优先级 Large > Medium > Small)。
- **warmup retry 误投递告警丢失**:v1.13.30 warmup retry 命中文字但路由错时 fall through 到 timeout 诊断,弱化 misroute 告警。新增 `if v2.matched && !routed2` 独立分支返回 `DeliveryMisrouted` + telemetry,跟首次 attempt 同等强度。
- **orchestrate per-row timeout 60s 短于 warmup 最坏 71s**:warmup FSEvents 60s + 0.8s settle + verify 10s ≈ 71s,outer 60s 先 fire → orchestrate 标记 `bridge_unavailable` 但 daemon 仍跑 → 下个 poll cycle 重复发送风险。`SEND_ROW_TIMEOUT` 60 → 120s 给 warmup retry 留 headroom。

**codex round-2 review v1.13.30 NICE:**

- **`wait_for_message_db_change` 不 filter `message_*.db`**:父目录任意文件写入(session.db / fts / temp / 等)都唤醒,可能浪费唯一 retry 配额。`event.paths` 检查 basename 含 `message_`,空兜底(notify backend 可能 emit 无 paths 事件)。

测试:213 全 lib pass。e2e:`doctor` query_ready=true / send_ready=false 正确反映,stale socket `kill -9` 后自动重启,Auto-resolved=small 显示真实。

**留 v1.13.32+:**
- `history --strip-xml` / `--fields` 防 appmsg XML 撑爆 agent context(codex CLI 反馈 #3)
- `wechat digest <chat>` 内置摘要(大 feature,codex CLI 反馈 #6)
