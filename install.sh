...
prompt_tcc_grant_via_dialog() {
  ...
  # v1.15.0+: stop telling users to drag binaries. macOS has a perfectly
  # good native dialog (Raycast / Rectangle / Karabiner all use this):
  # `AXIsProcessTrustedWithOptions(prompt=YES)` from inside each binary
  # fires the standard "App is requesting access" prompt with an
  # "Open System Settings" button — and registers the binary's identity
  # in the Accessibility list automatically. The user only flips the
  # switch (no drag).
  #
  # The API is per-CALLER, so bridge AND wechatd must each fire their
  # own prompt. We also reset stale TCC entries from prior install
  # versions so the new prompt isn't ignored due to a cdhash mismatch
  # against a hidden old grant.
  # v1.16.0+: TCC keys on CFBundleIdentifier (from .app/Info.plist) for
  # bundled binaries. Both wechatd + wechat-bridge live inside the same
  # WechatSkillHelper.app, so ONE reset clears both. Also reset legacy
  # codesign-identifier-keyed entries from v1.15.x for users upgrading
  # across the bundle migration.
  if tccutil reset Accessibility ai.wechatskill.helper >/dev/null 2>&1; then
    info "已清理 WechatSkillHelper.app Accessibility 旧 entry"
  fi
  ...
  info "弹原生 macOS Accessibility 授权对话框 (wechat-bridge + wechatd 各一次)"
  "${bridge}" --request-trust >/dev/null 2>&1 || true
  "${wechatd}" request-trust >/dev/null 2>&1 || true
  open "x-apple.systempreferences:com.apple.preference.security?Privacy_Accessibility" 2>/dev/null || true

  # Live poll instead of "press button when done": every 1.5s bounce the
  # LaunchAgent + check /health. Once 200, both binaries inherited TCC
  # and we're done. Cap at ~120s so a forgotten install doesn't hang.
  local elapsed=0
  local interval=2
  local cap=120
  local last_status=""
  while (( elapsed < cap )); do
    ...
    if wait_for_bridge_health 2>/dev/null; then
      return 0
    fi
    local cur="elapsed=${elapsed}s"
    if [[ "$cur" != "$last_status" ]]; then
      info "等待 Accessibility 授权… 在 macOS 弹出的对话框点【Open System Settings】,把 wechat-bridge / wechatd 的开关打开 (${elapsed}s / ${cap}s)"
      last_status="$cur"
    fi
    sleep "$interval"
    elapsed=$(( elapsed + interval + 1 ))
  done
  warn "120s 内没等到 Accessibility 授权。可在 System Settings → 辅助功能 把 wechat-bridge / wechatd 开关打开后,跑 wechat doctor 复验"
  return 1
}
...
for BIN_NAME in "${BINS[@]}"; do
  ...
  # `--force` re-signing.
  # v1.16.0+: wechatd + wechat-bridge are packaged into the
  # WechatSkillHelper.app bundle below. Skip per-binary codesign for
  # those — the bundle's `codesign --deep` re-signs them with the
  # bundle's stable CFBundleIdentifier (ai.wechatskill.helper). Doing
  # both wastes time AND rotates cdhash twice (per-binary then bundle).
  if [[ "${BIN_NAME}" == "wechatd" || "${BIN_NAME}" == "wechat-bridge" ]]; then
    success "已安装：${INSTALL_DIR}/${BIN_NAME}（codesign 由 .app bundle 处理）"
    continue
  fi
...
# v1.16.0+: Wrap wechatd + wechat-bridge in a .app bundle so macOS treats
# them as a real app with a stable CFBundleIdentifier. Without this, TCC
# Accessibility grants for unbundled CLI binaries are flaky:
#   • Settings shows "wechat-bridge" / "wechatd" entries that look ON but
#     aren't actually granted (TCC.db has no row).
#   • `tccutil reset Accessibility ai.wechatskill.<bin>` silently no-ops
#     because the third arg must be a CFBundleIdentifier — codesign
#     `--identifier` on a raw binary is NOT a CFBundleIdentifier.
#   • cdhash rotation across versions invalidates the prior grant; the
#     visual entry persists but binding is dead → user keeps re-toggling
#     a stale row and send keeps failing.
# Solution per codex review (2026-05-07): one .app bundle owns BOTH
# binaries. CFBundleIdentifier=ai.wechatskill.helper, LSUIElement=1.
# ~/.local/bin/{wechatd,wechat-bridge} become symlinks into the bundle so
# PATH usage + LaunchAgent ProgramArguments keep working unchanged.
HELPER_APP="$HOME/Applications/WechatSkillHelper.app"
HELPER_MACOS="${HELPER_APP}/Contents/MacOS"
HELPER_BUNDLE_ID="ai.wechatskill.helper"
...
<dict>
  ...
  <key>CFBundleName</key><string>WechatSkillHelper</string>
  <key>CFBundleDisplayName</key><string>WechatSkill Helper</string>
  <key>CFBundleExecutable</key><string>wechatd</string>
  <key>CFBundlePackageType</key><string>APPL</string>
  <key>CFBundleShortVersionString</key><string>${VERSION:-1.16.0}</string>
  <key>CFBundleVersion</key><string>${VERSION:-1.16.0}</string>
  <key>LSUIElement</key><true/>
  <key>LSMinimumSystemVersion</key><string>13.0</string>
</dict>
</plist>
EOF
# Move (or link) wechatd + wechat-bridge into the bundle. Use `mv` to
# preserve inode + xattrs from the just-installed binary at
# ${INSTALL_DIR}/${b} (which has correct content + perms). Then symlink
# back so PATH / LaunchAgent / `wechat-bridge --check-trust` keep working.
for b in wechatd wechat-bridge; do
  src="${INSTALL_DIR}/${b}"
  dst="${HELPER_MACOS}/${b}"
  if [[ -L "${src}" ]]; then
    # Already a symlink from prior upgrade — the install loop above
    # replaced it with a fresh regular file, but only if INSTALL_DIR is
    # writable. For root-owned dirs the install ran via sudo and the
    # symlink may have re-appeared. Re-resolve.
    rm -f "${src}"
  fi
  if [[ -f "${src}" ]]; then
    mv -f "${src}" "${dst}"
  elif [[ -f "${dst}" ]]; then
    : # already in place from a previous run; mv source missing is fine
  else
    warn "${b} 既不在 ${src} 也不在 ${dst},install 流程异常"
    continue
  fi
  ln -sf "${dst}" "${src}"
done
# Drop quarantine on the bundle (ad-hoc signed apps can still pick up
# the quarantine xattr on first download via curl).
xattr -dr com.apple.quarantine "${HELPER_APP}" 2>/dev/null || true
# Codesign the WHOLE bundle as one identity. --deep ensures the inner
# binaries inherit the bundle's signature + identifier. We re-sign on
# every install because the binary contents changed; the bundle id
# stays stable so TCC grants persist (this is the whole point).
CODESIGN_BUNDLE_ERR=$(mktemp "${TMPDIR:-/tmp}/wechat-install-bundle-cs.XXXXXX")
if codesign --force --deep --sign - --identifier "${HELPER_BUNDLE_ID}" "${HELPER_APP}" 2>"${CODESIGN_BUNDLE_ERR}"; then
  if codesign --verify --deep "${HELPER_APP}" 2>>"${CODESIGN_BUNDLE_ERR}"; then
    success "已打包 WechatSkillHelper.app (bundle id ${HELPER_BUNDLE_ID})"
  else
    warn "WechatSkillHelper.app codesign --verify 不通过:"
    sed 's/^/    /' "${CODESIGN_BUNDLE_ERR}" >&2
  fi
else
  warn "codesign 打包 WechatSkillHelper.app 失败:"
  sed 's/^/    /' "${CODESIGN_BUNDLE_ERR}" >&2
fi
rm -f "${CODESIGN_BUNDLE_ERR}"
