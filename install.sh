...
prompt_tcc_grant_via_dialog() {
  ...
  if tccutil reset Accessibility ai.wechatskill.wechat-bridge >/dev/null 2>&1; then
    info "已清理 wechat-bridge Accessibility 旧 entry"
  fi
  if tccutil reset Accessibility ai.wechatskill.wechatd >/dev/null 2>&1; then
    info "已清理 wechatd Accessibility 旧 entry"
  fi
